...
def test_year_api_error_empty():
    input = ""
    expected_output = "non input"
    response = client.get("/service/getage?year="+input)
    assert response.status_code == 422

def test_year_api_error_non_input():
    input = ""
...
def test_year_api_input_more_current_year():
...
def test_year_api_input_less_zero():
    input = "-1"
    expected_output = "input less zero"
    response = client.get("/service/getage?year="+input)
    assert response.status_code == 200
    assert response.json() == {"error": expected_output}